...
using Coevery.Metadata.Records;
...
namespace Coevery.Metadata.Controllers {
    public class FieldDependencyController : ApiController {
        ...
        // GET api/metadata/FieldDependency?entityName=lead
        ...
        public virtual HttpResponseMessage Post(string entityName, string controlFieldName, string dependentFieldName, JObject value) {
            ...
            var dependencies = value.GetValue("Dependencies");
            if (dependencies == null) {
                return Request.CreateResponse(HttpStatusCode.NotFound);
            }
            var fieldDependencies = dependencies.ToArray();
            foreach (var fieldDependency in fieldDependencies) {
                var dependencyRecord = new FieldDependencyRecord {
                    Entity = partDefinition,
                    ControlField = controlField,
                    DependentField = dependentField,
                    ControlFieldValue = fieldDependency.Value<string>("ControlFieldValue"),
                    DependentFieldValue = fieldDependency.Value<string>("DependentFieldValue")
                };
