...
namespace Coevery.Metadata {
    public class NavigationMenu : INavigationProvider
    {private readonly IContentDefinitionManager _contentDefinitionManager;
        private readonly IContentManager _contentManager;

        public NavigationMenu(IContentDefinitionManager contentDefinitionManager, IContentManager contentManager)
        {
                    ...
                    foreach (var contentTypeDefinition in contentTypes) {
                        ...
                        if (createRouteValues.Any())
                            menu.Add(T(contentTypeDefinition.DisplayName), "5", item => item.Action(cim.CreateRouteValues["Action"] as string, cim.CreateRouteValues["Controller"] as string, cim.CreateRouteValues)
                                ...
                                .Permission(DynamicPermissions.CreateDynamicPermission(DynamicPermissions.PermissionTemplates["PublishOwnContent"], contentTypeDefinition)));
                    }
                });
            //}
        }
