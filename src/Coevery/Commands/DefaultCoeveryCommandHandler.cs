...
namespace Coevery.Commands {
    public abstract class DefaultCoeveryCommandHandler : ICommandHandler {
        protected DefaultCoeveryCommandHandler() {
        ...
        private void SetSwitchValue(KeyValuePair<string, string> commandSwitch) {
            ...
            if (propertyInfo.GetCustomAttributes(typeof(CoeverySwitchAttribute), false).Length == 0) {
                throw new InvalidOperationException(T("A property \"{0}\" exists but is not decorated with \"{1}\"", commandSwitch.Key, typeof(CoeverySwitchAttribute).Name).Text);
            }
        ...
        private void CheckMethodForSwitches(MethodInfo methodInfo, IDictionary<string, string> switches) {
            ...
            foreach (CoeverySwitchesAttribute switchesAttribute in methodInfo.GetCustomAttributes(typeof(CoeverySwitchesAttribute), false)) {
