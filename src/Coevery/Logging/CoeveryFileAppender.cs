...
namespace Coevery.Logging {
    public class CoeveryFileAppender : RollingFileAppender {
        /// <summary>
            ...
            lock (this) {
                ...
                for (int i = 1; !fileOpened && i <= Retries; i++) {
                    try {
                        ...
                        LogLog.Error(typeof(CoeveryFileAppender), string.Format("CoeveryFileAppender: Failed to open [{0}]. Attempting [{1}-{2}] instead.", fileName, fileName, newSuffix));
                    }
                }
