...
namespace Coevery.Packaging.Commands {
    public class PackagingCommands : DefaultCoeveryCommandHandler {
    ...
    The default filename is Coevery.[Module|Theme].<extensionName>.<extensionVersion>.nupkg.
    For example, ""package create SampleModule c:\temp"" will create the package
    ""c:\temp\Coevery.Module.SampleModule.1.0.0.nupkg"".")]
        ...
        public void CreatePackage(string extensionName, string path) {
            ...
            if (packageData == null) {
                Context.Output.WriteLine(T("Module or Theme \"{0}\" does not exist in this Coevery installation.", extensionName));
                return;
            }

            // append "Coevery.[ExtensionType]" to prevent conflicts with other packages (e.g, TinyMce, jQuery, ...)
    ...
    The <packageId> should take the format Coevery.[Module|Theme].<extensionName>.
    For example, ""package uninstall Coevery.Module.SampleModule"" will uninstall the Module under the ""~/Modules/SampleModule"" directory and
    ""package uninstall Coevery.Theme.SampleTheme"" will uninstall the Theme under the ""~/Themes/SampleTheme"" directory.")]
