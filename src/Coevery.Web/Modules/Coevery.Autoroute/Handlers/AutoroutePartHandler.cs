...
namespace Coevery.Autoroute.Handlers {
    public class AutoroutePartHandler : ContentHandler {
        ...
        private void PublishAlias(AutoroutePart part) {
            ...
            if (part.DisplayAlias == "/") {
                ...
                var currentHomePages = _coeveryServices.ContentManager.Query<AutoroutePart, AutoroutePartRecord>().Where(x => x.DisplayAlias == "").List();
        ...
        private void ProcessAlias(AutoroutePart part) {
            ...
            // should it become the home page ?
            if (part.DisplayAlias != "/" && _coeveryServices.Authorizer.Authorize(Permissions.SetHomePage)) {
                ...
                if (!_autorouteService.Value.ProcessPath(part))
                    _coeveryServices.Notifier.Warning(T("Permalinks in conflict. \"{0}\" is already set for a previously created {2} so now it has the slug \"{1}\"",
